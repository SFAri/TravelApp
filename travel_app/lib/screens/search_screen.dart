...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Gọi hàm filter để lọc danh sách ngay khi dữ liệu thay đổi
    _filterDestinations(widget.query);
  }

  void _filterDestinations(String query) {
    final filtered = destinations.where((destination) {
      return AppLocalizations.of(context)!.getString(destination['nameKey'].toLowerCase()).contains(query.toLowerCase());
    }).toList();
